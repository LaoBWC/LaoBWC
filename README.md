...
一、2D形状识别算法研究。  针对项目的具体问题，我们进行了基于SVM的2D形状识别的尝试。针对要分类的几何形状，我们自己制作了10类几何形状，其分别是十字形，三角形，长方形，菱形，梯形，五边形，五角星，圆，椭圆，扇形。每类的样本数目为10，然后把这些样本送到SVM中进行训练。我们直接调用了opencv中集成的机器学习库，然后只要对其中的函数参数进行设置既可以了。各种形状如下图：
![img](http://b.hiphotos.baidu.com/image/pic/item/8644ebf81a4c510f184a552d6a59252dd42aa57f.jpg)

   我们采用了简单的基于图像像素的特征，SVM的类型为 C_SVM,核函数为LINEAR,迭代次数为10000次，精度0.001。在测试部分，我们自己又制作了10类每类2个样本进行测试。最终的测试结果如下：
![img](http://f.hiphotos.baidu.com/image/pic/item/4610b912c8fcc3cefefb7f049845d688d43f20c9.jpg)

![img](http://h.hiphotos.baidu.com/image/pic/item/d833c895d143ad4b8eb4f0f788025aafa40f066a.jpg)

实验结果分析：
    从上述结果可以看出，SVM的分类效果还可以，前面几个都分对了，就是后面两个分错了。针对这些分错的结果，分析发现分类器会把它们都分为五边形，这说明前面的特征还是太简单了，应当选取更具有表达力的特征；其次，我们的训练样本也比较少，这可能导致分类器没有学习到足够有区分度的特征。所以后续的改进要从这两个方面展开。 
